...
public class PortalKeyItem : Item
{
    ...
    [SerializeField] private GameObject pocketInventoryInstance;
    #endregion

    #region InternalVars
    private Transform _playerCamera;
    private GameObject _lastKeyOwner; // player that used the item to teleport into the pocket inventory.
                                      // used for returning the key if it's left inside the pocket inventory accidentally. 
    ...
    private List<GameObject> _playersInPocket = new List<GameObject>(); // teleports all players out on return()
    ...
    public override void OnPickUp(GameObject user)
    {
        ...
        _lastKeyOwner = user; // set the last key owner to the player that picked it up.
        if (user == null) {
            Debug.LogError("PortalKey_MONO: PickUp() user is null.");
            return;
        }
        _playerCamera = user.GetComponent<Inventory>().orientation;
    }
    ...
    public void AddRaycastToTp(float distance, LayerMask layerMask){
        if (_playerCamera == null) {   
            Debug.LogError ("PortalKey_MONO: RaycastFromCamera() camera is null.");
            return;
        }

        RaycastHit[] hits = Physics.RaycastAll(_playerCamera.position, _playerCamera.forward, distance, layerMask);
