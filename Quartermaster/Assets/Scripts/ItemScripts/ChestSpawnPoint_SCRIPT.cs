...
public class ChestSpawnPoint : NetworkBehaviour {
    ...
    public void SpawnChestAtSelfServerRpc() {
        ...
        //raycast down from spawn point to find closest whatIsGround layer object, and place chest on top

        Vector3 spawnPosition = transform.position + Vector3.up * 0.5f; // Adjust the spawn position to be above the spawn point

        GameObject chest = Instantiate(_chestPrefab, spawnPosition, transform.rotation);
