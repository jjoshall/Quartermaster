...
public class RailgunItem : Item
{
    ...
    public void fire(GameObject user)
    {
        ...
        SlowTargets(targetsHit, _slowPotency, _slowDuration);
    ...
    private void SlowTargets(List<Transform> targetsHit, float slowAmount, float slowDuration)
    {
        foreach (Transform target in targetsHit)
        {
            if (target != null)
            {
                SlowTarget(target, slowAmount, slowDuration);
            }
        }
    }

    private void SlowTarget(Transform target, float slowAmount, float slowDuration)
    {
        BaseEnemyClass_SCRIPT enemy = target.GetComponent<BaseEnemyClass_SCRIPT>();
        if (enemy != null)
        {
            enemy.ApplyTimedSlowServerRpc(slowAmount, slowDuration);
        }
        else
        {
            Debug.LogWarning("Tried to slow a target that is not an enemy: " + target.name);
        }
    }
